...
import { ExcelJson } from '../sharedModels/excel-json.interface';
...
export class ExportService {
   ...
   */
   public exportToCsv(rows: object[], fileName: string, columns?: string[], title?: string[]): string {
    if (!rows || !rows.length) {
      return;
    }
    const separator = ',';
    
    let csvContent = title.join(separator) + '\n';
