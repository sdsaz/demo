...
import { PrimeTemplate, PrimeNGConfig, TranslationKeys, OverlayService } from 'primeng/api';
...
export class CustomCalenderComponent implements OnInit, OnDestroy, ControlValueAccessor {
  ...
  /* CUSTOM FEATURE */
  @Input() showRangeButtonBar: boolean;

  @Input() dateSelectOptionsButtonBar: string[] = [];
  ...
  ngOnInit() {
    ...
    let firstDateMeta = { day: this.rangeDates[0].getDate(), month: this.rangeDates[0].getMonth(), year: this.rangeDates[0].getFullYear(), otherMonth: this.rangeDates[0].getMonth() !== this.currentMonth || this.rangeDates[0].getFullYear() !== this.currentYear, today: false, selectable: true };
    if(this.rangeDates[1]!=null){
    let secondDateMeta = { day: this.rangeDates[1].getDate(), month: this.rangeDates[1].getMonth(), year: this.rangeDates[1].getFullYear(), otherMonth: this.rangeDates[1].getMonth() !== this.currentMonth || this.rangeDates[1].getFullYear() !== this.currentYear, today: false, selectable: true };
    this.onDateSelect(null, secondDateMeta);
  }
    this.onDateSelect(null, firstDateMeta);
  }
    if (this.view === 'date') {
      this.createWeekDays();
      this.initTime(date);
  ...
  selectDate(dateMeta) {
    ...
    if (this.isSingleSelection()) {
      ...
      if (this.value && this.value.length) {
        ...
        if (!endDate && startDate && date.getTime() >= startDate.getTime()) {
          endDate = date;
        }
        else {
          startDate = date;
  ...
  onUserInput(event) {
    ...
    try {
      ...
      //this.updateModel(value); // Commented method to prevent key press call untill valid date -- PN 21/07/2022
    }
  ...
  isValidSelection(value): boolean {
    ...
    if (this.isSingleSelection()) {
      ...
      if (this.isRangeSelection()) {
        if(value != null && (value[1] != null || value[1] != undefined)){
  ...
  /* CUSTOM START FUNCTIONS FOR BOTTOM BUTTON BAR */
  UpdateDateRange(event, value) {
    this.updateModel(value);
    this.updateInputfield();
    this.updateUI();
  ...
  onRangeTodayButtonClick(event) {
    const date: Date = new Date();
    this.UpdateDateRange(event, [date, date]);
  }

  onRangeYesterdayButtonClick(event) {
    const date: Date = new Date(this.dateAddDays(-1));
    this.UpdateDateRange(event, [date, date]);
  }

  onRangeLast7DaysButtonClick(event) {
    const firstDate: Date = new Date(this.dateAddDays(-6));
    const currentDate: Date = new Date();
    this.UpdateDateRange(event, [firstDate, currentDate]);
  }

  onRangeNext7DaysButtonClick(event) {
    const firstDate: Date = new Date();
    const currentDate: Date = new Date(this.dateAddDays(6));
    this.UpdateDateRange(event, [firstDate, currentDate]);
  }

  onRangeThisWeekButtonClick(event) {
    const firstDateOfWeek = this.getFirstDateOfWeek();
    let firstDate: Date = new Date();
    for (let index = 0; index < 7; index++) {
      firstDate = new Date(this.dateAddDays(-index));
      if (firstDateOfWeek === firstDate.getDay()) {
        break;
      }
    }
    const currentDate: Date = new Date();
  ...
  onRangeNextWeekButtonClick(event) {
    const firstDate: Date = new Date();
    const currentDate: Date = new Date(this.dateAddDays(6));
    this.UpdateDateRange(event, [firstDate, currentDate]);
  }

  onRangeLast30DaysButtonClick(event) {
    const firstDate: Date = new Date(this.dateAddDays(-29));
    const currentDate: Date = new Date();
    this.UpdateDateRange(event, [firstDate, currentDate]);
  }

  onRangeNext30DaysButtonClick(event) {
    const currentDate: Date = new Date();
    const nextDate: Date = new Date(this.dateAddDays(29));
    this.UpdateDateRange(event, [currentDate, nextDate]);
  }

  onRangeThisMonthButtonClick(event) {
    const today = new Date();
    const firstDate: Date = new Date(today.getFullYear(), today.getMonth(), 1, 0, 0, 0, 0);
    const currentDate: Date = new Date();
    this.UpdateDateRange(event, [firstDate, currentDate]);
  }

  onRangeNextMonthButtonClick(event) {
    const today = new Date();
    const firstDate: Date = new Date(today.getFullYear(), today.getMonth() + 1, 1, 0, 0, 0, 0);
    const currentDate: Date = new Date(today.getFullYear(), today.getMonth() + 2, 0, 0, 0, 0, 0);
    this.UpdateDateRange(event, [firstDate, currentDate]);
  }

  dateAddDays(noOfDays) {
    return new Date(Date.now() + (86400000 * noOfDays));
  }

  showHideDateSelectionOption(currentOption) {
    let showOption: boolean = false;
    if (this.dateSelectOptionsButtonBar) {
      this.dateSelectOptionsButtonBar.forEach((item) => {
        if (item.toLocaleLowerCase() === currentOption.toLocaleLowerCase())
          showOption = true;
      });
    }
    return showOption;
  }
  /* CUSTOM END FUNCTIONS FOR BOTTOM BUTTON BAR */
