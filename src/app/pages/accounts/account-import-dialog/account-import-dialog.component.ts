...
@Component({
  selector: 'ngx-account-import-dialog',
...
export class AccountImportDialogComponent {
      ...
      this.labelImport.nativeElement.innerText = Array.from(files)
        .map(f => {
          if (!f.name.endsWith('.xlsx') && !f.name.endsWith('.xls')) {
            this._commonHelper.showToastrError(this._commonHelper.getInstanceTranlationData('CRM.ACCOUNT.IMPORT_DIALOG.FILE_FORMAT_ERROR'));
  ...
  getTemplateFile() {
    this._accountService.downloadImportTemplate(this.entityWorkflowId ?? 0).then((base64String: any) => {
      ...
      link.download = this._commonHelper.getConfiguredEntityName('{{Accounts_plural_p}}') + '_' + this._commonHelper.getInstanceTranlationData('CRM.ACCOUNT.IMPORT_DIALOG.DOWNLOAD_ENTITY_FILE_POSTFIX') + '_' + moment().format("YYYY-MM-DDTHH:mm:ss") + '.xlsx';
      link.click();
  ...
  importBulkAccounts(): void {
    if (this.fileInput.nativeElement.files.length == 0) {
      this._commonHelper.hideLoader();
      this._commonHelper.showToastrError(this._commonHelper.getInstanceTranlationData('CRM.ACCOUNT.IMPORT_DIALOG.SELECT_FILE_ERROR'));
      return;
    }
    ...
    this._accountService.importBulkAccounts(params)
      .then(() => {
        this._commonHelper.showToastrSuccess(this._commonHelper.getInstanceTranlationData('CRM.ACCOUNT.IMPORT_DIALOG.SUCCESS_MESSAGE'));
        this._commonHelper.hideLoader();
        this.onClose(true);
      }, (error) => {
        this._commonHelper.hideLoader();
